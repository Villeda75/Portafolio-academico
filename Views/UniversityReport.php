...
<head>
  ...
  <title>Reporte Universidad JOVESOLIDES</title>
    ...
    <section id="services" class="services section-bg">
      <div class="container">
        ...
        <header class="section-header"> 
          <h3>Reporte de Universidades</h3>
          <p>Visualice la cantidad de becarios por universidad.</p>
        </header>
        ...
        <script>
          ...
          var myChart = new Chart(ctx, {
              ...
              data: {
                labels: [
                      <?php
                        $sql = "SELECT Nombre as Uni FROM universidad"; //obtenemos todas las universidades
                        $consulta = mysqli_query(strConnection(),$sql);
                        while( $registros = mysqli_fetch_array($consulta) )
                        {
                      ?>
                        '<?php echo $registros["Uni"]?>',
                      <?php
                        }
                      ?>
                  ], //barras
                  datasets: [{
                      label: '# de becarios por universidad', //titulo del graáfico
                      ...
                      [
                        <?php 
                          while( $registros = mysqli_fetch_array($consulta) )
                          {
                              ...
                              INNER JOIN universidad ON universidad.Id_Universidad = formacion_academica.Id_Universidad) 
                              WHERE universidad.Nombre = '" .$registros['Uni'] . "'"; //consulta por cada universidad

                              $consulta1 = mysqli_query(strConnection(),$sql1);
                              $cantidad = mysqli_fetch_assoc($consulta1); //cantidad de becarios por cada universidad
                        ?>

                        <?php echo $cantidad['cant'] ?>, //imprimimos la cantidad de becarios x universidad
                        <?php
                          }
                        ?>
                      ], //frecuencia
